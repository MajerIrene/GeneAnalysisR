...
```{r}
choose_k(filtered_data, method = "elbow", max_k = 10)
```
...
```{r}
choose_k(filtered_data, method = "silhouette", max_k = 10)
```
...
hierarchical <- clustering(filtered_data, k = 2, method = 'hierarchical', distance = 'euclidean')
...
```
The clear separation suggests that hierarchical clustering has identified two groups that differ significantly along a dominant latent feature—possibly linked to biological or experimental conditions such as treated vs. untreated. The fact that PC1 captures the vast majority of the variance further supports the idea that a two-cluster solution is appropriate and meaningful in this dataset in both cases. We can now visualize the dendrogram resulting from the hierarchical clustering. In this package, the plot_dendrogram function omits gene labels by default, as displaying all of them would result in an unreadable and cluttered visualization.
```{r, warning=FALSE}
...
```
And let's do the same for hierarchical clustering
```{r}
plot_heatmap(filtered_data, hierarchical)
```
...
## Enrichment Analysis
Finally GeneAnalysisR provides two functions to perform enrichment analysis: go_enrichment and gsea_enrichment. So let's perform both the analysis to explore the results.
Let's start with go enrichment analysis. Please note: this process may take a moment to complete.
```{r, warning=FALSE}
go_kmeans <- go_enrichment(kmeans$clusters)
```
We can now visualize the results using the plot_dotplot function from GeneAnalysisR, which displays a dotplot of the top 10 most enriched GO terms.
```{r}
plot_dotplot(go_kmeans[[1]], title = "GO Enrichment - KMeans cluster 1")
```

```{r}
plot_dotplot(go_kmeans[[2]], title = "GO Enrichment - KMeans cluster 2")
```
A similar analysis can be performed for hierarchical and pam clustering, but it will not be covered in this vignette.

We can also perform Gene Set Enrichment Analysis in a slightly different way. GSEA, in fact, requires a ranked list of genes as input. In this package, we generate this ranking by calculating the log2 fold change of gene expression within each cluster compared to the rest of the samples. This approach allows us to capture cluster-specific differential expression patterns as the basis for enrichment analysis.
```{r}
